...
```plaintext
# ตอบคำถามที่นี่
one to many ในระบบจองห้องพัก แต่ละห้อง สามารถถูกจองได้หลายครั้งในช่วงเวลาต่าง ๆ กัน
...
```plaintext
# 1. Positive case : เช่น Login Success จะทดสอบการ login ด้วย username และ password admin/admin123 ซึ่งให้ API ตอบกลับ status 200 และส่ง JWT token กลับมา แสดงว่าระบบทำงานถูกต้อง

2. Negative case : ตัวอย่างเช่น Login Wrong Password จะทดสอบการ login ด้วยรหัสผ่านผิด admin/password ซึ่งให้ API ตอบกลับ status 401 (Unauthorized) เพื่อให้ระบบปฏิเสธการเข้าถึงเมื่อข้อมูลไม่ถูกต้อง
...
```plaintext
# ตอบคำถามที่นี่
เหตุผลที่ใช้ self-hosted runner ต้องการสเปคเครื่องที่สูงกว่ามาตรฐาน  ,ต้องการความเร็วในการ Build

เปรียบเทียบข้อดี-ข้อเสีย
ข้อดี สเปคปรับแต่งได้, รันงานได้รวดเร็ว ไม่ต้องโหลด Cache ใหม่, เข้าถึงระบบภายในได้

ข้อเสีย ต้องเสียเวลาดูแลเซิร์ฟเวอร์เอง อัปเดต OS ลบไฟล์ขยะ, จะเกิดปัญหาได้ถ้าใช้ กับ Public Repo
...
ต้องติดตั้ง helmet ก่อน
npm install helmet
curl -I https://booking-backend-jsfe.onrender.com/api/rooms
